...
if (isset($_POST['Locality']))
{
	...
	else
	{
		if (isset($_POST['Area']))
		{
			...
			if($myArea == "All"){
				$sql = "SELECT * from $table where Locality = '$myLocality' order by Locality, St_Name, H_No";
			}
			else{
				$sql = "SELECT * from $table where Locality = '$myLocality' and Area = '$myArea' order by Locality, St_Name, H_No";
			}
...
function myFunction(locality, area, zoomid) {
	...
	//alert(myarea);
	//alert(zoomid);
	var url = "https://"+window.location.host+"/mobile/map.php?area="+myarea+"&zoomid="+zoomid+"&locality="+mylocality+"&Action=Submit";
    window.location = url;
}
  ...
  <div data-role="main" class="ui-content">
	...
	<tr>
		<td align="middle">
			...
			<form method="POST">
				...
				$mySQL = "Select distinct Locality from $table where Coordinates != '' order by Locality";
					//echo $mySQL;
	...
	<table >
	  ...
	  foreach($faq as $k=>$v) {
		  ...
		  $googlelink = "<a href='https://maps.google.com/?q=".$myAddress."'>".$myAddress."</a>";
		  $editlink = "<a href='https://myjoula.com/mobile/functions.php?Action=lastvisit&ID=".$faq[$k]['ID']."' target='_new'>Edit Comments</a>";
